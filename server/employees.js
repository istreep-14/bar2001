// Only writer of the employees table: the people you work with. A real table, not a pick list, because it will
// keep gaining columns (today: name, role, notes). A name is unique ignoring case, adding a name that exists
// returns that person (un-archived if need be), and a person who appears on a shift is archived instead of
...
const noop = () => {};
const shape = (row) => (row ? { ...row, archived: !!row.archived } : null);
...
export function employeeStore(db, { emit = noop } = {}) {
  ...
  const store = {
    list({ includeArchived = false } = {}) {
      return db.prepare(`SELECT * FROM employees ${includeArchived ? '' : 'WHERE archived = 0'} ORDER BY name`).all().map(shape);
    },
    get: (id) => shape(one('SELECT * FROM employees WHERE id = ?', id)),
    ...
    summary() {
      ...
      for (const r of rows) {
        const t = byId.get(r.employee_id) ?? {
          ...
          first_worked: r.work_date, last_worked: r.work_date,
        };
        t.shifts += 1;
        if (r.start_at) {
          t.timed_shifts += 1;
        ...
        if (r.work_date < t.first_worked) t.first_worked = r.work_date;
        if (r.work_date > t.last_worked) t.last_worked = r.work_date;
    ...
    // the other fields are used when the person is new and otherwise left as they are.
    ensure({ name, role = null, notes = null }) {
      ...
      const id = randomUUID();
      db.prepare('INSERT INTO employees (id, name, role, notes) VALUES (?, ?, ?, ?)').run(id, name, role, notes);
    ...
    update(id, patch) {
      ...
      if (keys.length) {
        const bound = Object.fromEntries(keys.map((k) => [k, typeof patch[k] === 'boolean' ? Number(patch[k]) : patch[k] ?? null]));
