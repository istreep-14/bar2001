...
- **Shifts:** newest first, grouped by month (each month's heading carries its shifts, hours, tips and total), one compact row each: the **date** (with the shift-type badge, a **Party** badge when the shift had one, and the location), **time worked** (the hours, leading; when the shift ran is the detail underneath), then **Tips** and **Total income** (tips + estimated wage + other). The rest is one arrow away, in the row that opens under a shift: **wage** (hours × rate), **other income** by type, the **crew** (bartender count and hours, staff tips, each person's hours and tips), party, breaks, notes and tags, and **Show raw data**. Amounts and per-hour figures are **rounded to whole dollars in the table; hover any of them for the exact cents**. Every per-hour figure is over hours worked (the shift's length minus its breaks). A dash means nothing was entered. Hovering a row's date gives its crew, tags, notes and party. On the right of each row are three icon buttons: **edit** (pencil), **details** (arrow) and **delete** (bin; asks twice: the first click turns it into a red *Delete?* for four seconds; it is a soft delete). Clicking elsewhere on a row does nothing. "Show deleted" reveals soft-deleted shifts, which can be edited and restored from the form.
...
1. **Date:** tap a day on a month calendar that shows the shifts you have already logged (type and tips on each day, and a note if the day you pick already holds one). You can also type the date.
...
3. **Type:** **Day**, **Night** or **Double**, each card showing what your tips per hour has averaged on it. It's required and never guessed, so choose one.
...
6. **Misc:** other income, as entries with a **type** you choose (Cash, Venmo, Paycheck…), same amount format and same Day / Night / Combined choice on a double. Add and rename the types under **Lists → Misc types**.
...
**Nothing pops up, and it isn't one long scroll.** The page is two panels. On the left is tabbed navigation in section groups; on the right is the full input for the page you picked. While browsing, the groups are **Browse** (Overview, Calendar, Shifts, Live feed) and **Lists** (Locations, Misc types, Wage rates), where the same three lists can be edited without opening a shift. While a shift is open they are:
